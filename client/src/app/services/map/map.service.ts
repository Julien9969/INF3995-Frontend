import {Injectable} from '@angular/core';
import {SocketService} from "@app/services/socket/socket.service";
import {BehaviorSubject} from "rxjs";

@Injectable({
  providedIn: 'root'
})
export class MapService {
  private observable: BehaviorSubject<HTMLImageElement>;

  constructor(readonly socket: SocketService) {
    const image = new HTMLImageElement();
    image.src = "data:image/png;base64,"
    this.observable = new BehaviorSubject<HTMLImageElement>(image)

    this.socket.on('map-data', (bitmapBase64: string) => {
      // Check if image is in correct base64 format
      if (!bitmapBase64.startsWith('data:image/bmp;base64,')) {
        console.error('Invalid image format');
        return;
      }
      // Create image
      const image = new HTMLImageElement();
      image.onload = async () => {
        this.observable.next(image);
      }
      image.src = bitmapBase64;
    });
  }

  get bitmapObservable() {
    return this.observable.asObservable();
  }

}
